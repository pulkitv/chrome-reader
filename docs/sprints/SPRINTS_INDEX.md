# ReadEasy Sprint History Index

> Sprint cadence: 1 month per sprint
> Coverage start: January 2026
> Last updated: May 25, 2026

This index tracks month-by-month delivery history so humans and CLI agents can quickly review what was shipped, why it was prioritized, and what changed.

## Sprint files

1. [2026-01 — Foundation + Reader Core](2026-01-sprint-foundation-reader-core.md)
2. [2026-02 — Reading Experience Expansion](2026-02-sprint-reading-experience-expansion.md)
3. [2026-03 — Reading List Scale + X4 Delivery](2026-03-sprint-reading-list-and-x4-delivery.md)
4. [2026-04 — Sidepanel Rearchitecture + Floater Reliability](2026-04-sprint-sidepanel-rearchitecture-and-floater.md)
5. [2026-05 — May 2026 (FB Extraction, Edit Mode, PDF, Reader Refactor, ChatGPT, Context Menu, Auth Fix)](2026-05-sprint.md)

## How to use this folder

- Read the sprint file for a month to understand:
  - Why initiatives were executed
  - What was implemented
  - Architectural or UX impact
  - Relevant commits (when known)
- For deep implementation details, cross-reference:
  - [../PROJECT_SUMMARY.md](../PROJECT_SUMMARY.md)
  - [../PROJECT_ARCHITECTURE.md](../PROJECT_ARCHITECTURE.md)
  - [../PROJECT_ARCHITECTURE_FOUNDATION_AND_SYSTEM_MAP.md](../PROJECT_ARCHITECTURE_FOUNDATION_AND_SYSTEM_MAP.md)
  - [../PROJECT_ARCHITECTURE_WORKFLOWS_AND_HISTORY.md](../PROJECT_ARCHITECTURE_WORKFLOWS_AND_HISTORY.md)

## Going forward

- Use [SPRINT_TEMPLATE.md](SPRINT_TEMPLATE.md) to create each new monthly sprint file.
- Recommended naming format: `YYYY-MM-sprint-<focus>.md`
