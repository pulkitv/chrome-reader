...
- **Source-page captures originate in the extension** — the Chrome extension is the only surface that can scrape an `http(s)://` source page, so first captures of real URLs always come from the extension. For an `articles` row whose `url` is a real `http(s)://` URL, the web app should read/version it but should not create it.
- **User-initiated drafts may be created by the web app** — drafts that don't correspond to a source page (notes, blank starts, pastes) are created with `url = 'webapp://draft/<uuid>'` and `site_name = 'Web App'`. The same `(google_uid, url)` uniqueness contract holds — pick a fresh `<uuid>` per draft. These rows participate in versioning like any other article.
- **Extension consumers (if cloud-pull is later added)** should treat `webapp://draft/...` rows as display-only entries — don't try to open them in a browser tab, don't re-extract, and prefer `site_name` over a derived hostname for the label.
